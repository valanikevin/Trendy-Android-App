package com.aiora.trendy.providers.instagram;
...
public class InstagramFragment extends Fragment  implements InfiniteRecyclerViewAdapter.LoadMoreListener {
	...
	private class DownloadFilesTask extends AsyncTask<String, Integer, ArrayList<InstagramPhoto>> {
		...
		protected void onPreExecute() {
			...
			if (initialload) {
				if (noAcccessToken(mAct))
				    nextpageurl = "https://www.instagram.com/" + username + "/media/";
			    else
	...
	public ArrayList<InstagramPhoto> parseJson(JSONObject json) {
		...
		try {
			...
			} else if (noAcccessToken(mAct)) {
				nextpageurl = "https://www.instagram.com/" + username + "/media/?max_id=" + json.getJSONArray("items")
						.getJSONObject(json.getJSONArray("items").length() - 1).getString("id");
			} else {
				nextpageurl = null;
			}

			// parsing json object
			 JSONArray dataJsonArray = json.getJSONArray(noAcccessToken(mAct) ? "items" : "data");
	...
	public static boolean noAcccessToken(Context context){
